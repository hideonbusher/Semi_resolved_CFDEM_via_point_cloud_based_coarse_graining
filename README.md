...
Two force-model variants are provided: CGlucy12Dgidaspow is intended for small-scale simulations, while CGlucy15Dgidaspow is designed for large-scale parallel simulations. Select only one according to the computational scale; both must be used with CGVoidFraction.
...
The virtual mass force model is implemented by Guo's Model, working on DEM side.
